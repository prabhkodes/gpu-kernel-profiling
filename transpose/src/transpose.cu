...
#ifndef TILE_DIM
#define TILE_DIM 16      // override at compile time: -DTILE_DIM=32
#endif
static_assert(TILE_DIM * TILE_DIM <= 1024,
              "TILE_DIM*TILE_DIM exceeds the 1024 threads-per-block limit; "
              "the launch will fail silently at runtime.");
...
int main(int argc, char **argv) {
    // Traces in ../traces were captured at N = 1000 (4 MB int32 matrix).
    int width  = (argc > 1) ? std::atoi(argv[1]) : 1000;
    int height = width;
    ...
    nvtxRangePush("Copy1, Slow");
    launchMatrixCopy(d_in, d_out, width, height); // slower, meant for profiling
    nvtxRangePop();


    nvtxRangePush("Copy2, Fast");
    launchMatrixCopy(d_in, d_out, width, height); // faster ,meant for profiling
    nvtxRangePop();
    

    nvtxRangePush("Transpose Unshared");
    launchMatrixTranspose(d_in, d_out, width, height);
    nvtxRangePop();


    nvtxRangePush("Transpose Shared Memory");
